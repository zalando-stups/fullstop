...
@Component
public class PieronePlugin extends AbstractFullstopPlugin {

    private static final Logger LOG = LoggerFactory.getLogger(PieronePlugin.class);
    ...
    @Value("${fullstop.plugins.pierone.url}/blubblubblub/{appId}")
    private String pieroneApplicationUrl;

    @Autowired
    public PieronePlugin(final ClientProvider cachingClientProvider) {
    ...
    private String getSourceLocation(final CloudTrailEvent event) {
        ...
        if (!matcher.find()) {
            LOG.error("No source defined for this instance {}, " +
